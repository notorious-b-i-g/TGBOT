...
async def delete_order(callback: types.CallbackQuery, state: FSMContext):
    ...
    order_status_check = await get_data(query, (order_id,))
...
async def send_message_to_worker(message: types.Message, state: FSMContext):
    ...
    query = """
           ...
           """
    await insert_data(query, client_name, worker_name, message_content, send_time, worker_name, client_name, order_id)
